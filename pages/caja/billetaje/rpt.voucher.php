<?php
  ...
  $detalle = "";
  $importeLetras = "";
  $sql = "select d.*,pr.nombre as producto,mv.nombre as tipo_mov,upper(fn_get_letras(importe)) as importe_letras1,upper(' con '||round(importe-trunc(importe),2)) as importe_letras2 from bn_movim_det d join bn_productos pr on d.id_producto=pr.id join sis_mov mv on d.id_tipo_mov=mv.id where id_movim=$1 order by item";
  $qry = $db->query_params($sql,array($movimID));
  if ($db->num_rows($qry)) {
