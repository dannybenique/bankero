let rutaSQL = "pages/coopsud/ahorrosX/sql.php";
  ...
  appAjaxSelect(datos,rutaSQL).done(function(resp){
    if(resp.length>0){
      ...
      $.each(resp,function(key, valor){
        ...
        fila += '<td style="text-align:center;"><a href="javascript:appAhorrosCambiarNumCert(\''+((valor.codsocio)+"."+(valor.tipo_serv)+"."+(valor.numcert))+'\');">'+(valor.numcert)+'</a></td>';
...
function appAhorrosCambiarNumCert(cuenta){
  let numcert = prompt("Ingrese el nuevo certificado a cambiar en la cuenta..."+(cuenta));

  if(numcert!=null){
    let datos = {
      TipoQuery:"coopSUDcorregirNumCert",
      numcert : numcert,
      cuenta : cuenta
    }
    appAjaxSelect(datos,rutaSQL).done(function(resp){
...
function appAhorrosView(codsocio,tipo_serv,num_cert){
  ...
  appAjaxSelect(datos,rutaSQL).done(function(resp){
    if(resp.usernivel==resp.admin){ //permisos
      let _html='';
      //if(resp.DPF.saldo!=resp.socio.saldo) { _html +='<button type="button" id="btn_CorregirDPF" class="btn btn-social btn-google btn-sm" onclick="javascript:appCorregirDPFsaldo();" title="Corregir Saldo en DPF"><i class="fa fa-flash"></i> saldo DPF:'+(appFormatMoney(resp.DPF.saldo,2))+'</button> ';}
      $("#div_reservado").html(_html);
    }
...
function appLlenarMovimientos(tabla){
  ...
  $.each(tabla,function(key, valor){
    ...
    fila += '<td><input type="checkbox" name="chk_Borrar" id="chk_Borrar" value="'+(valor.num_trans)+'"/></td>';
    fila += '<td>'+(valor.agencia)+'</td>';
    fila += '<td>'+(valor.ventanilla)+'</td>';
    fila += '<td style="text-align:center;"><a href="javascript:appShowModalImporteMovim(\''+(valor.agencia+"."+valor.ventanilla+"."+valor.num_trans)+'\')">'+(valor.num_trans)+'</a></td>';
  ...
  fila += '<tr>';
  fila += '<td colspan="7" style="text-align:right;font-weight:bold;">Total</td>';
  ...
  fila += '</tr>';
  fila += '<tr>';
  fila += '<td colspan="7" style="text-align:right;font-weight:bold;">Saldo movim</td>';
...
function appShowModalImporteMovim(voucher){
  let datos = {
    TipoQuery : "coopSUDgetImporteMovim",
    voucher   : voucher
  }
  appAjaxSelect(datos,rutaSQL).done(function(resp){
    $("#txt_movimFecha").val(moment(resp.fecha.date).format("DD/MM/YYYY"));
    $("#hdn_movimHora").val(resp.hora);
    $("#lbl_movimVoucher").html(resp.voucher);
    $("#txt_movimImporte").val(appFormatMoney(resp.importe,2));
...
function modCambiarImporte(){
  if(confirm("Esta seguro de cambiar este importe??")){
    let datos = {
      TipoQuery : "coopSUDcambiarImporteMovim",
      voucher   : $("#lbl_movimVoucher").html(),
      importe   : appConvertToNumero($("#txt_movimImporte").val())
    }
    appAjaxSelect(datos,rutaSQL).done(function(resp){
      //console.log(resp);
      appRefresh();
...
function modCambiarFecha(){
  if(confirm("Esta seguro de cambiar este fecha??")){
    let datos = {
      TipoQuery : "coopSUDcambiarFechaMovim",
      voucher   : $("#lbl_movimVoucher").html(),
      fecha   : appConvertToFecha($("#txt_movimFecha").val(),"-")+(" ")+($("#hdn_movimHora").val())
    }
    appAjaxSelect(datos,rutaSQL).done(function(resp){
      //console.log(resp);
      appRefresh();
