<?php
  ...
  switch ($data->TipoQuery) {
    case "selCreditos":
      ...
      $qry = $db->query_all("select * from vw_prestamos_min where estado=1 and saldo>0 ".$whr." limit 25 offset 0;");
      if ($db->num_rows($qry)) {
    ...
    case "viewCredito":
      //cabecera
      $prestamo = 0;
      $qry = $db->query_all("select * from vw_prestamos_ext where id=".$data->prestamoID);
      if ($qry) {
        $rs = reset($qry);
      ...
      //detalle
      $detalle = array();
      $qry = $db->query_all("select *,capital+interes+seguro+gastos as total from bn_prestamos_det where id_prestamo=".$data->prestamoID);
      if ($qry) {
        foreach($qry as $rs){
          $detalle[] = array(
            "numero" => $rs["numero"],
